...
from hutil.Qt.QtWidgets import QHBoxLayout, QComboBox, QSizePolicy, QAbstractSpinBox,  QSpinBox, QPushButton,  QCheckBox, QGraphicsView, QWidget, QDoubleSpinBox, QLabel, QVBoxLayout,  QGraphicsScene,  QGraphicsItem, QGraphicsRectItem
from hutil.Qt.QtGui import QColor, QFont, QTransform, QCursor, QPixmap, QPainter, QPen, QIcon, QBrush, QPainterPath
from hutil.Qt.QtCore import QEvent, QTimer, QSize,  QPointF, QLineF,  QPoint, Qt, QRect, QRectF, QPropertyAnimation
...
class eltween(QtWidgets.QFrame):
    ...
    __easeTypes = ["Linear","Power1","Power2","Power3","Power4", "Elastic", "Bounce","Expo"]
    ...
    def _setupGlobals(self):  
        self.elGlobal = dict(
            ...
            #object_tracking
            elBoxes=[],
            #this is a 2d array for each track has all the sub objects
            timeline_items=[], 
            #this is like the items array but it tracks in terms of numbers 
    ...
    def setCurrentNode(self, node): 
        if node is not None and node.type().name().startswith("elTween"): 
            ...
            for b in range(boxLen):
                if self.elGlobal["elBoxes"][b].highlighted:
                    newSize = QRectF(self.elGlobal["elBoxes"][b].boundingRect()) 
    ...
    def addTrackGuide(self): 
        ...
        text.setFont(  QFont("Helvetica [Adobe]", textFontSize, weight=textFontWeight))
        text.setX(8)
    ...
    def drawingSceneLayout(self):
        ...
        self.elGlobal["view"].setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    ...
    def TabsBar(self):
        ...
        trashButtonSpacer.setSizePolicy(   QSizePolicy.Expanding, QSizePolicy.Expanding)
        ...
        self.elGlobal["tabButtons"][0].setStyleSheet(self.elGlobal["CSS"]["tabButtonOn"][0])
        ...
        ##Create Frame for each
            ...
            for key in tabProps[t]:  
                ...
                if indx is len(tabProps) and t is tabCount-1: 
    ...
    def createProperty(self,type, id, optionalArray=None):    
        ...
        if id == "atprops_0": heading.setText("Overshoot")
        if id == "atprops_1" : heading.setText("Amplitute")
    ...
    def updateProperty(self, event, id):  
            ...
            if b.highlighted == True:  
                if event != b.exportables[id]:  # new info 
                    ...
                    if id == "duration" or id == "delay":
                        ...
                        self.elGlobal["elTweenLayout"].addFrames(b, b.track, b.subTrack)
                        if id=="duration":  b.duration(event)
                        if id == "delay": b.delay(event)

                        self.elGlobal["elTweenLayout"].removeFrames(b, b.track, b.subTrack)
        ...
        #Add visual guide something is different
...
##    ##    ##    ##     ## ##    ##     ##    
 ######     ##    ##     ## ##     ##    ##     
    ...
    def createLayout(self):  
        ...
        self.elGlobal["elTweenLayout"].size.setHeight(self.elGlobal["timelineHeightPX"]) 
...
#### ##     ## ##         #######  ##     ##    ##
    ...
    def importTool(self): 
        ...
        if str_data is not None:
            rawJSON = json.loads(self.elGlobal["node"].userData("boxExportables") )
            #populate new data
...
class elTweenLayoutidget(QWidget): 
    ...
    def resizeEvent(self, event): 
        ...
        if self.elGlobal["view"]:  # if youve hooked the graphics view in resize it
            ...
            self.elGlobal["size"][0] = (self.size.width() - (buttonSizeWidth*2)) 
    ...
    def addTween(self, track, importMode=False, subTrack=-1): 
        ...
        for i in range(framecount): 
            if freeI == minFreeFrames:
                ...
                break
            #SUB TRACK CHECK
            if subTrack is not -1: 
                #check only the current subtrack
                if tl_track[i] is not -1:  # start checking for space
                    freeI += 1
            #MAIN TRACK CHECK
            elif subTrack is -1 and trackX[i] is not -1 and trackY[i] is not -1 and trackZ[i] is not -1:
                #this is a main track so check if any of the 3 subtracks have something in them
                freeI += 1
    ...
    def checkSettings(self): 
        firstFrameChange = self.elGlobal["startFrame"] != hou.playbar.frameRange()[0]-1
        lastFrameChange = self.elGlobal["endFrame"] != hou.playbar.frameRange()[1]
    ...
    def updateSettings(self):   
        ...
        currentFrames = int(hou.playbar.frameRange()[1]-hou.playbar.frameRange()[0]+1)
       ...
       # newFrames = self.elGlobal["frames"]
        ...
        self.elGlobal["default_tween_pixels"]= self.elGlobal["timeline_width"] / \
            ...
            self.elGlobal["default_tween_length"]/hou.fps()  # TEMP , DELETE THS /2   
        self.elGlobal["scene"].setSceneRect(
            0, 0, self.elGlobal["timeline_width"], self.elGlobal["timeline_width"]) 
        self.elGlobal["timeline_height"]= self.size.height() /self.elGlobal["tracks"]
        ...
        for bar in self.elGlobal["elBoxes"]:
            ...
            #timeline is being rescaled so we have to somehow update the bounding box of each box
...
class elTimeline(QGraphicsView):
    ...
    def boxRelease(self, event):  # letting go of a box
        if self.__boxSelected:
            self.__boxSelected.release(event, 1, self.elGlobal["elTweenLayout"])
        #EXECUTE LAST
...
class addSubTweenButton(QPushButton):
    ...
    def setup(self, elGlobal, id):
        ...
        self.elGlobal = elGlobal
        if self.id<3: self.prop = 0
        elif self.id<6: self.prop = 1
        elif self.id<9: self.prop = 2
        else:  self.prop = 3 #im too dumb to think of a modulo for this
        onCSS = "border: 15px solid #"
        ...
        self.cssOFF = offCSS+self.BorderColor +";" + elGlobal["CSS"]["subTweenOff"]
...
class draggableBox(QGraphicsItem):
    ...
    def info(self): 
        ...
        self.__outerColor = QColor(css["colorStroke"][self.exportables["track"]])
        ...
        self.exportables["x"] = 0 if self.__track is not 2 else 1
        self.exportables["y"] = 0 if self.__track is not 2 else 1
        self.exportables["z"] = 0 if self.__track is not 2 else 1
    ...
    def delay(self, amount=None):  
        if amount is not None:
            ...
            if self.exportables["duration"]+self.exportables["delay"] > (float(self.elGlobal["frames"])):
                self.exportables["delay"] = float(self.elGlobal["frames"]) - self.exportables["duration"]
    ...
    def duration(self, amount=None):
        ...
        if amount is not None: 
            ...
            if self.exportables["duration"]+self.exportables["delay"] > (float(self.elGlobal["frames"])):
                self.exportables["duration"] = float(self.elGlobal["frames"]) - self.exportables["delay"]
            self.__width = (self.exportables["duration"] ) / \
                (float(self.elGlobal["frames"])) * self.elGlobal["scene"].width()
        ...
        if self.__drag == True:
            ...
            if self.__edgeSide == 0:  # drag mode
                ...
                if self.__edgeSide == 1:  # Right side selected
                    ...
                    self.elGlobal["objectsOnStage"].edgeLine.setZValue(21)
                    if self.init_duration+dragFrame + self.init_x > float(self.elGlobal["frames"]):  # RIGHT EDGE CAP
                        self.duration(float(self.elGlobal["frames"]) - self.init_x)
                if self.__edgeSide == -1:  # left side selected
                    if self.init_duration-dragFrame > self.elGlobal["minimumFrames"]:
                        if(self.__posX < 0):
                            ...
                            self.elGlobal["objectsOnStage"].edgeLine.setX(self.__pos.x())
                            self.elGlobal["objectsOnStage"].edgeLine.setZValue(21)
                if self.duration() < self.elGlobal["minimumFrames"]:  # cap drag to min
    ...
    def release(self, event, zoomFactor, main):
        ...
        for n in neighbours:
            ...
            if collision and self is not n:  # take out self and check if collision
                #COLLISION STATE
                #OVERLAP
...
def exportTool(context): 
    ...
    for t in range(context.elGlobal["tracks"]): 
        setupCode.append(hou.node(subnet.path()+'/el_tween_setup_CODE_'+str(t)))
